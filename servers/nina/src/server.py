...
def create_server(spec: dict, base_url: str) -> FastMCP:
    """ """
    mcp = FastMCP.from_openapi(
        openapi_spec=spec, client=httpx.AsyncClient(base_url=base_url)
    )
    ...
    @mcp.prompt(
        name="ars_lookup",
        description="ARS-Codes nachschlagen"
    )
...
1. Ermittle alle Mitarbeiter aus der Mitarbeiterdatenbank, die ihren Wohnsitz in Deutschland haben.
